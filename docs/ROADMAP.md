...
8. **Configurare e collaudare email e Google Calendar — `In corso` (P0).** Le credenziali Zimbra risultano disponibili ma non sono state lette né configurate. Il 30 luglio è stata approvata e implementata un'unica integrazione Google Calendar API per lettura e scrittura, eliminando la dipendenza dall'URL iCal. Dal pannello Google sono stati verificati progetto Cloud, API abilitata, account di servizio, chiave e condivisione del calendario operativo con permesso di modifica senza gestione della condivisione; il calendario resta non pubblico. Arzamed usa lo stesso account Google e ha attivo il modulo di sincronizzazione esterna. Il JSON locale è escluso da Git, non tracciato e protetto con permessi `600`; il contenuto non è stato letto. Lettura giornaliera, ricorrenze espanse, paginazione, eventi per l'intera giornata, cache, scope limitato agli eventi e fallback sono coperti dalla suite completa di 166 test Python e 18 test JavaScript. Restano inserimento sicuro dei valori nell'ambiente autorizzato e prove reali controllate con Arzamed, email e Calendar. **Criterio di uscita:** prove reali riuscite e anomalie registrate in `RegistroEvento`.
...
13. **Verificare la resilienza dei flussi — `In corso` (P0).** Il 29 luglio i test sintetici locali hanno confermato che errori SMTP non perdono prenotazioni, iscrizioni corso, iscrizioni e presenze del percorso nascita o call sonno; errori di scrittura Calendar non perdono conferme o call. Il 30 luglio la lettura API-only ha confermato il riuso della cache disponibile e la registrazione controllata dell'anomalia quando Google non risponde. Le anomalie vengono registrate in `RegistroEvento`, sono visibili con avvisi comprensibili nell'admin e non espongono credenziali Google nei log. Conferme, modifiche e annullamenti di prestazioni e call non dichiarano più “comunicazione inviata” quando l'email fallisce. Resta il collaudo controllato della stessa matrice in preproduzione con le integrazioni reali. **Criterio di uscita:** prenotazioni e iscrizioni restano salvate, gli errori secondari sono registrati e l'admin riceve avvisi comprensibili.
