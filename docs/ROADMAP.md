...
4. **Creare e verificare le migrazioni Alembic — `Completato` (P0).** La baseline `56dda7f5137f` crea tutte le tabelle, relazioni, unicità e indici; l'avvio operativo usa `flask db upgrade` e non `db.create_all()`. **Evidenza:** SQL PostgreSQL revisionato, upgrade ripetuto su database vuoto e adozione senza perdita di uno schema rappresentativo popolato verificati il 20 luglio 2026. **Criterio di uscita:** soddisfatto.
...
15. **Finalizzare contenuti e autorizzazioni — `In corso` (P1).** Il placeholder pubblico della landing sonno è stato sostituito il 21 luglio 2026 e nessun template pubblico richiama immagini placeholder. Restano da verificare autorizzazioni e provenienza delle fotografie, qualifiche OPI, fonti cliniche e testimonianze reali. **Criterio di uscita:** nessun contenuto pubblico privo di autorizzazione o ancora provvisorio.
