...
8. **Configurare e collaudare email e Google Calendar — `In corso` (P0).** Le credenziali Zimbra risultano disponibili ma non sono state lette né configurate. Il 30 luglio è stata approvata e implementata localmente un'unica integrazione Google Calendar API per lettura e scrittura, eliminando la dipendenza dall'URL iCal. Il repository documenta la procedura dell'account di servizio ma non contiene evidenza che l'identità tecnica sia già stata creata o che il calendario operativo le sia stato condiviso. Lettura giornaliera, ricorrenze espanse, paginazione, eventi per l'intera giornata, cache, scope limitato agli eventi e fallback sono coperti dalla suite completa di 166 test Python e 18 test JavaScript. Restano creazione e configurazione esterne, inserimento sicuro dei valori e prove reali autorizzate con Arzamed, email e Calendar. **Criterio di uscita:** prove reali riuscite e anomalie registrate in `RegistroEvento`.
