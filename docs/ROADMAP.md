...
4. **Creare e verificare le migrazioni Alembic — `Completato` (P0).** La baseline `56dda7f5137f` crea tutte le tabelle, relazioni, unicità e indici; la revisione `9b7e2d4c6a10` aggiunge qualificazione, UTM e stato dei promemoria email della call sonno, mentre `7f3c1a2d9e40` rimuove i campi WhatsApp non più utilizzati. L'avvio operativo usa `flask db upgrade` e non `db.create_all()`. **Evidenza:** SQL PostgreSQL revisionato, upgrade su database vuoto, adozione senza perdita e `flask db check` senza operazioni mancanti verificati. **Criterio di uscita:** soddisfatto.
...
6. **Preparare la configurazione di produzione — `In corso` (P0).** Validazione fail-fast, HTTPS/proxy, PostgreSQL obbligatorio, matrice dei segreti, SMTP Zimbra, secret file Calendar e log senza indirizzi email sono predisposti e testati localmente. Il 29 luglio sono stati rimossi i fallback Gmail, resi obbligatori server/porta/TLS e casella Zimbra approvati, aggiunto `PUBLIC_BASE_URL` per canonical e link assoluti e introdotto l'opt-in separato per le integrazioni reali nella preproduzione privata. La ricognizione Render dello stesso giorno ha confermato che lo staging contiene soltanto la configurazione minima gratuita: mancano ancora tutte le variabili SMTP, Calendar, origine pubblica e opt-in preproduzione; è inoltre rimasta una variabile WhatsApp non più usata. Restano da inserire e verificare i valori reali nel pannello Render. **Criterio di uscita:** avvio corretto senza segreti nel repository e log privi di dati personali superflui.
7. **Pubblicare uno staging privato — `In corso` (P0).** Lo staging HTTPS su Render conserva Basic Auth, indicizzazione disabilitata e PostgreSQL collegato in Francoforte, ma la ricognizione del 29 luglio ha rilevato una deriva: il servizio e il Blueprint risultano collegati a `codex/modern-header`, mentre il deploy manuale attivo usa il commit `8e42c3c` di `codex/snap-homepage`; l'auto-deploy è disattivato. Il controllo `/healthz` viene interrogato da Render ogni cinque secondi e finisce sotto il limite globale, producendo `429` e riavvii periodici. L'esenzione dal rate limiting è stata implementata e testata localmente; restano deploy del commit verificato, rimozione delle variabili `ADMIN_BOOTSTRAP_*`, controllo dei dati esclusivamente sintetici e nuovo collaudo live. **Criterio di uscita:** staging privato stabile, branch e commit intenzionali, bootstrap rimosso e controlli Render superati.
...
12. **Collaudare il flusso della consulenza del sonno — `In corso` (P0).** Prenotazione qualificata, disponibilità del sabato, UTM e promemoria email sono predisposti. Restano le prove reali di Calendar, email, conferma, modifica, annullamento, call conclusa, pagamento e questionario privato. **Criterio di uscita:** flusso completo verificato con integrazioni reali e disponibilità nazionale invariata.
13. **Verificare la resilienza dei flussi — `In corso` (P0).** Il 29 luglio i test sintetici locali hanno confermato che un errore SMTP non perde la prenotazione, un errore di scrittura Calendar non perde la conferma e un errore di lettura Calendar usa la cache disponibile; tutti vengono registrati in `RegistroEvento` senza esporre l'URL iCal nei log. Restano il collaudo controllato della matrice completa in preproduzione e la verifica degli avvisi admin per ciascun flusso. **Criterio di uscita:** prenotazioni e iscrizioni restano salvate, gli errori secondari sono registrati e l'admin riceve avvisi comprensibili.
14. **Validare privacy, cookie e trattamento dati — `Da fare` (P0).** Il 29 luglio il contatto e la forma dell'indirizzo postale sono stati allineati ai dati operativi approvati. Restano da verificare informative, basi giuridiche, responsabili esterni, consensi, conservazione, Analytics, questionario sonno, collegamenti esterni e dati sanitari con competenza professionale adeguata. **Criterio di uscita:** testi e comportamento approvati; GA4 e Meta non ricevono eventi prima del consenso.
