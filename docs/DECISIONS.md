...
## D-101 — I preload dei font seguono il peso usato nel primo rendering

- Data: 2026-08-29.
- Stato: approvata, implementata e verificata localmente.
- Decisione: precaricare nel login admin Atkinson Hyperlegible 400 e Bricolage Grotesque 700, che corrispondono rispettivamente al testo e al titolo visibili. Conservare sulla pagina pubblica il preload di Bricolage Grotesque 800, effettivamente usato nel primo rendering del sito.
- Motivo: il login precaricava il file ExtraBold 800 ma applicava al titolo il peso 700, provocando il warning del browser per una risorsa precaricata e non utilizzata entro pochi secondi.
- Conseguenze: font, file distribuiti, palette e resa grafica non cambiano; viene corretto soltanto il suggerimento di caricamento anticipato. I test verificano separatamente i preload della homepage e del login.
- Collegamenti: `templates/base.html`, `templates/login.html`, `static/css/fonts.css`, `tests/test_app.py`, D-094.

## D-102 — La privacy policy usa l’informativa estesa aggiornata

- Data: 2026-08-29.
- Stato: approvata dal committente e implementata localmente; resta necessaria la coerenza operativa dei consensi e dei tempi di cancellazione.
- Decisione: sostituire la sintesi pubblicata su `/privacy` con il testo integrale fornito il 29 agosto 2026, articolato in venti sezioni ai sensi degli artt. 13 e 14 GDPR. Conservare integralmente titolare e recapiti, categorie di dati, finalità e basi giuridiche, partner e minori, immagini, marketing, WhatsApp, PayPal, Calendar/ArzaMed, professionisti, destinatari, trasferimenti, conservazione, cookie, diritti e reclamo.
- Motivo: la pagina precedente era incompleta, riportava una base giuridica generica non coerente per l’erogazione sanitaria, citava ancora Google Fonts come servizio esterno e applicava indistintamente 24 mesi a trattamenti con termini ora differenziati.
- Conseguenze: la pagina chiarisce che il trattamento sanitario necessario si fonda sull’art. 9, par. 2, lett. h) GDPR, mentre consenso esplicito e revoca restano collegati alle finalità per cui sono richiesti. La resa HTML mantiene un solo `h1`, usa sottosezioni leggibili e rende cliccabili email, telefono e Garante. La pubblicazione del testo non implementa automaticamente cancellazioni periodiche, doppio consenso genitoriale per immagini di minori o nuovi strumenti promozionali e di pagamento; questi aspetti richiedono verifica operativa separata. Non cambia lo schema del database.
- Collegamenti: `templates/privacy.html`, `static/css/internal-pages.css`, `CONTENT_AND_ASSETS.md`, `tests/test_app.py`, D-097.

## D-103 — Moduli, autorizzazioni e conservazione seguono la nuova informativa

- Data: 2026-08-29.
- Stato: approvata dal committente, implementata e verificata localmente; resta necessaria la validazione professionale dell’informativa e delle procedure.
- Decisione: trattare le checkbox obbligatorie dei moduli ordinari come presa visione dell’informativa, senza presentarle come consenso generale al trattamento. Mantenere separati e obbligatori i consensi espliciti necessari per i dati della gravidanza e le dichiarazioni sanitarie richieste ai fini della sicurezza. Raccogliere le autorizzazioni facoltative a fotografie e riprese soltanto nella scheda amministrativa dell’iscrizione, per singola persona, finalità e canale, registrando per i minori entrambi gli esercenti la responsabilità genitoriale oppure la responsabilità esclusiva. Registrare revoche e consegna in presenza dell’informativa al partner o secondo partecipante. Applicare quotidianamente i termini di conservazione dichiarati mediante cancellazione o anonimizzazione dei dati identificativi scaduti.
- Motivo: un consenso privacy indistinto non descrive correttamente trattamenti fondati su contratto, obbligo o finalità sanitaria; immagini, dati particolari e comunicazioni facoltative richiedono evidenze distinte. L’informativa deve inoltre corrispondere al comportamento effettivo del gestionale, compresa la gestione delle revoche e dei tempi di conservazione.
- Conseguenze: la revisione Alembic `b7d2e4f6a810` aggiunge la tracciabilità del consenso ai dati della gravidanza, della consegna dell’informativa ai terzi, delle autorizzazioni immagini e dell’avvenuta anonimizzazione. I moduli pubblici non raccolgono più un’autorizzazione immagini generica; la relativa scheda amministrativa consente registrazione granulare e revoca. Le richieste di revoca indicate nell’informativa possono essere inviate a `info@scstudioinfermieristico.it`. Il job giornaliero applica 6 mesi alle richieste non seguite da rapporto, 12 mesi a corsi e consulenze educative concluse e 24 mesi ai dati organizzativi di appuntamenti conclusi, preservando i riepiloghi anonimi e le evidenze ancora necessarie. Upgrade completo e `flask db check` sono riusciti su un database SQLite vuoto; la suite passa con 260 test Python e 37 test JavaScript. Informativa, modulo open day e scheda amministrativa popolata sono stati verificati a 1440×900 e 390×844 px senza overflow o errori console, con aree checkbox e azioni mobile da almeno 44 px. L’implementazione tecnica non costituisce parere legale e non chiude il gate di validazione professionale.
- Collegamenti: `app.py`, `templates/privacy.html`, `templates/iscrizione_corso.html`, `templates/iscrizione_accompagnamento_privata.html`, `templates/admin_dettaglio.html`, `OPERATIONS.md`, `SITE_MAP_AND_FLOWS.md`, `ROADMAP.md`, `migrations/versions/b7d2e4f6a810_consensi_immagini_e_conservazione.py`, `tests/test_app.py`, `tests/test_migrations.py`, D-097, D-102.

## D-104 — Le iscrizioni ai corsi richiedono l’accettazione delle condizioni

- Data: 2026-08-30.
- Stato: approvata dal committente e implementata localmente; deploy ancora richiesto.
- Decisione: pubblicare su `/condizioni-iscrizione-corsi` il testo integrale fornito il 30 agosto 2026 e richiedere la checkbox `Dichiaro di aver letto e accettato le Condizioni di iscrizione ai corsi.` in tutti i moduli che inviano una richiesta di iscrizione a un corso: BLSD, disostruzione pediatrica, open day, laboratori e percorso nascita privato. La raccolta di interesse e i flussi del percorso sonno e della call sonno restano esclusi. Verificare l’accettazione anche lato server e registrare nella pratica la versione e l’istante dell’accettazione.
- Motivo: la persona deve poter consultare prima dell’invio le regole su conferma del posto, pagamento in presenza, rinuncia, mancata presentazione, spostamenti e variazioni del corso; il solo attributo HTML `required` non impedirebbe invii costruiti senza browser.
- Conseguenze: il link si apre in una nuova scheda per non perdere i dati già compilati. Le pratiche storiche e gli inserimenti amministrativi senza evidenza restano riconoscibili come accettazione non registrata. Non cambia lo schema del database: versione e istante sono conservati nei dati strutturati già associati all’iscrizione. L’implementazione tecnica non costituisce parere legale e non sostituisce la validazione professionale del testo.
- Collegamenti: `app.py`, `templates/condizioni_iscrizione_corsi.html`, `templates/_condizioni_iscrizione_corsi_checkbox.html`, `templates/iscrizione_corso.html`, `templates/iscrizione_accompagnamento_privata.html`, `templates/admin_dettaglio.html`, `SITE_MAP_AND_FLOWS.md`, `CONTENT_AND_ASSETS.md`, `tests/test_app.py`, D-088, D-102, D-103.

## D-105 — Il percorso sonno usa una proposta privata e condizioni versionate

- Data: 2026-08-31.
- Stato: approvata dal committente, implementata e verificata localmente; validazione professionale, configurazione dei pagamenti reali e deploy ancora richiesti. Sostituisce prezzi, rate interne e criteri di rimborso di D-033 e D-034; integra il flusso questionario di D-020.
- Decisione: pubblicare le condizioni della consulenza sonno versione `2026-08-31` e applicare i prezzi 75 € per la Consulenza mirata, 200 € per il Percorso sonno personalizzato e 320 € per il Percorso sonno con affiancamento. Dopo la call Selene invia una proposta privata valida sette giorni, revocabile, rigenerabile e reinviabile: per il percorso la famiglia sceglie tra la versione senza o con WhatsApp. Prima del pagamento vengono registrate l’accettazione obbligatoria delle condizioni e una richiesta separata e facoltativa di avvio prima dei 14 giorni. Il questionario viene inviato in entrambi i casi subito dopo la conferma manuale del pagamento; senza avvio anticipato, analisi e lavoro professionale attendono 14 giorni. Il pagamento è integrale; l’eventuale “Paga in 3 rate” è esterno e dipende da PayPal.
- Motivo: formula, prezzo, condizioni applicabili e scelta sul recesso devono essere dimostrabili prima del passaggio al gestore di pagamento. I link PayPal non sono necessari per costruire e collaudare il flusso e possono essere aggiunti successivamente tramite configurazione, senza modificare codice o contratto.
- Conseguenze: la revisione Alembic `c9e1f4a7b260` aggiunge proposta, pagamento, accettazioni, fase del percorso e attivazione WhatsApp alla call sonno. L’admin riconcilia manualmente PayPal o bonifico, quindi invia riepilogo, condizioni PDF e questionario. Il rimborso base del percorso è 200 €, 150 €, 100 €, 50 € o zero secondo la fase; al percorso con affiancamento si sommano 120 €, 90 €, 60 €, 30 € o zero secondo l’attivazione e i giorni WhatsApp. La terza call deve svolgersi entro 75 giorni dall’invio del questionario compilato. I valori reali PayPal e i dati bancari restano fuori dal repository. Upgrade su database sintetico, 278 test Python, 37 test JavaScript, controllo visuale desktop/mobile e verifica dei tre PDF sono riusciti. L’implementazione tecnica non costituisce parere legale.
- Collegamenti: `sleep_terms.py`, `app.py`, `config.py`, `templates/condizioni_consulenza_sonno.html`, `templates/offerta_sonno.html`, `templates/admin_dettaglio.html`, `migrations/versions/c9e1f4a7b260_offerta_e_pagamento_sonno.py`, `SITE_MAP_AND_FLOWS.md`, `PROJECT_BRIEF.md`, `CONTENT_AND_ASSETS.md`, `OPERATIONS.md`, `ROADMAP.md`, `tests/test_app.py`, `tests/test_migrations.py`, D-020, D-033, D-034, D-102, D-103.

## D-106 — Il flusso sonno non applica limiti anagrafici e rende esplicita la scelta di pagamento

- Data: 2026-08-31.
- Stato: approvata dal committente e implementata localmente; deploy e configurazione dei pagamenti reali ancora richiesti. Sostituisce D-004, D-035 e le parti anagrafiche di D-019, D-033 e D-069; integra D-105.
- Decisione: rimuovere dalle pagine e dalla richiesta iniziale della consulenza i riferimenti a fasce d’età e non usare l’età come criterio di ammissione. Rendere facoltativo l’obiettivo della call. Nella proposta privata descrivere l’azione come scelta della modalità di pagamento; dopo la scelta del bonifico mostrare soltanto intestatario, IBAN e causale composta da formula e cognome del prenotante, lasciando un comando esplicito per modificare la scelta. Alla compilazione del questionario salvare il dato e una nuova attività admin prima dell’invio dell’avviso email, indipendentemente dalla richiesta di avvio prima dei 14 giorni. Rendere disponibile `Leggi questionario` anche nella scheda della call e non mostrare un rimborso corrente calcolato automaticamente. Le tre pagine legali caricano il documento completo senza rivelazioni progressive. Nel ricontatto BLSD non richiedere la conferma finale di iscrizione né una dichiarazione sulle prove pratiche separata dalla dichiarazione sanitaria esplicita.
- Motivo: la richiesta iniziale deve restare accessibile senza una soglia anagrafica; la selezione del pagamento non equivale alla conferma dell’accredito; un questionario compilato richiede sempre un’azione visibile dello Studio, mentre il calcolo contestuale del rimborso non è utile nella scheda operativa. Nel ricontatto corso vanno raccolte soltanto le dichiarazioni pertinenti a quella fase.
...
- Data: 2026-09-13.
- Stato: approvata dal committente, implementata e verificata localmente; commit `edd8004` pubblicato su `origin/main`, stato del deploy da riconciliare separatamente.
...
## D-122 — Gli eventi Calendar entrano nello storico paziente soltanto dopo una decisione manuale

- Data: 2026-09-14.
- Stato: approvata dal committente, implementata e verificata localmente; migrazione e collaudo Render ancora richiesti.
- Decisione: estendere la ricerca amministrativa a Google Calendar nello stesso invio della ricerca locale, trasmettendo a Google il testo integrale inserito anche quando contiene telefono, email o codice fiscale e dichiarando questo comportamento vicino ai campi di ricerca. Nella scheda paziente, interrogare Calendar soltanto su comando esplicito e su tutto lo storico disponibile; nome e cognome generano esclusivamente possibili corrispondenze. Un evento entra nello storico del paziente solo dopo `Collega al paziente`, mentre `Non appartiene` viene memorizzato per non riproporlo. Conservare localmente soltanto ID tecnico dell’evento, paziente, decisione, amministratore e timestamp; titolo, data, orario e contenuti restano su Calendar. Mostrare in admin soltanto titolo, data e orario, mai la descrizione o le note Calendar.
- Motivo: lo storico Arzamed precedente al sito può essere utile nell’anagrafica, ma l’unicità di nome e cognome nel database locale non prova l’identità della persona descritta da un evento esterno. La conferma manuale e tracciata evita che un’omonimia diventi un’associazione sanitaria apparentemente certa. Il caricamento su richiesta mantiene inoltre Calendar una dipendenza secondaria rispetto alla scheda locale.
- Conseguenze: la tabella `calendar_patient_decision` conserva collegamenti e rifiuti senza copiare contenuti esterni; un evento può essere collegato a una sola persona, mentre le esclusioni restano specifiche per paziente. Le anagrafiche archiviate possono consultare lo storico ma non modificarlo. I risultati sono ordinati dal più recente e paginati; indisponibilità e letture parziali sono sempre dichiarate con un’azione di retry. Gli eventi riconoscibili come creati dal sito ma privi di collegamento locale non sono candidati paziente e generano una verifica operativa idempotente. L’anonimizzazione della persona elimina anche queste decisioni. Il downgrade della nuova revisione è consentito soltanto finché la tabella è vuota, per non cancellare decisioni già registrate. La trasmissione automatica del testo integrale a Google è una scelta esplicita del committente e richiede la relativa informativa sempre visibile nell’interfaccia. La suite completa passa con 453 test Python e 54 test JavaScript; i 7 test PostgreSQL opt-in passano inoltre su un cluster UTF-8 usa-e-getta. Il controllo visivo con dati sintetici a 1440×900 e 390×844 non rileva overflow; le nuove azioni hanno target da 44 px e la ricerca settimanale conserva visibili i limiti 07:00 e 22:00.
- Collegamenti: `app.py`, `templates/admin.html`, `templates/admin_paziente.html`, `static/css/admin.css`, `migrations/versions/e8b1c4d7a902_collegamenti_manuali_calendar_pazienti.py`, `SITE_MAP_AND_FLOWS.md`, `OPERATIONS.md`, `ROADMAP.md`, D-080, D-117, D-119, D-120, D-121.
