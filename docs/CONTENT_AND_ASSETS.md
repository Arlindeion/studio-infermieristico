...
- Avvisi Calendar in admin: il titolo prioritario è `Attenzione: Google Calendar è stato modificato`; ogni conflitto mostra il confronto `Sito`/`Calendar` e usa azioni esplicite (`Conferma dati del sito`, `Accetta data e orario Calendar`, `Ripristina su Calendar`, `Annulla appuntamento`). `Decidi dopo` chiarisce che conflitto e banner restano aperti.
