...
- Quando l’iscrizione viene collegata alla scheda paziente, il consenso privacy della richiesta viene registrato nello storico con stato, data di accettazione e riferimento alla specifica iscrizione. Un inserimento amministrativo senza consenso selezionato resta esplicitamente `Non registrato`.
...
- La richiesta pubblica conserva l’accettazione dell’informativa privacy. Quando l’appuntamento viene collegato a un paziente, la scheda mostra lo stato, la data dell’accettazione e l’appuntamento di origine. Un appuntamento inserito dall’admin registra l’accettazione soltanto se l’operatore seleziona esplicitamente `Consenso privacy raccolto`; in assenza di evidenza lo stato resta `Non registrata`.
